...
export function DownloadSection({ title, buttons }: DownloadSectionProps) {
    const getButtonClasses = (variant: string) => {
        ...
        if (variant === 'excel') {
            return `${baseClasses} bg-linear-to-r from-cyan-600 to-cyan-700 hover:from-cyan-500 hover:to-cyan-600 hover:shadow-cyan-500/50 focus:ring-cyan-500`
        } else if (variant === 'pdf') {
            return `${baseClasses} bg-linear-to-r from-rose-600 to-rose-700 hover:from-rose-500 hover:to-rose-600 hover:shadow-rose-500/50 focus:ring-rose-500`
        } else {
